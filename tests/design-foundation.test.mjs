...
test("About coach photo is a responsive image with explicit dimensions (CLS-safe)", () => {
  // The portrait is the real photo of Coach Myles with two of his players, shown
  // in warm color. It must still carry explicit width/height to avoid CLS.
  const img = html().match(/<img[^>]*alt="Coach [^"]*young baseball players[^>]*>/);
